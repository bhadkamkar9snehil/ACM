# ACM V8 - Task Backlog (CONSOLIDATED)

**Last Updated:** 2025-11-13  
**Focus:** Hands-Off, Self-Tuning, Continuous Adaptation

**Consolidation Status:** ✅ All audit documents consolidated into this master backlog
...
| Metric | Value | Notes |
|--------|-------|-------|
| **Active Tasks** | 61 | Including 36 new tasks from audit consolidation |
...
- Added Section 9: Regime Clustering Enhancements (8 tasks from Regime Audit)
- Added Section 10: Architecture & Code Quality (7 tasks from State Management & Comprehensive Audits)
- Total new tasks: 29 high-priority items from audit reviews
...
## 8 Forecast & AR(1) Model (FROM FORECAST AUDIT 2025-11-13)

**Source:** Forecast Audit.md - Critical statistical and algorithmic issues identified
...
## 9 Regime Clustering Enhancements (FROM REGIME AUDIT 2025-11-13)

**Source:** Regime Audit.md - Architecture and algorithm improvements

| ID     | Priority | Module            | Task                                                   | Completion Criteria                                                     | Status  |
| ------ | -------- | ----------------- | ------------------------------------------------------ | ----------------------------------------------------------------------- | ------- |
| REG-09 | High     | `core/regimes.py` | Remove k=1 fallback and rely on quality flagging       | Low-silhouette runs keep k >= 2 while quality flags surface in metadata | Planned |
| REG-10 | High     | `core/regimes.py` | Re-standardize feature basis & track PCA variance      | Combined basis normalized; PCA variance/explained logs emitted          | Planned |
| REG-11 | Medium   | `core/regimes.py` | Median smoothing & health-aware dwell enforcement      | Label smoothing uses median filter and dwell fixes prefer healthier IDs | Planned |
| REG-12 | High     | `core/regimes.py` | Robust transient detection state machine               | Startup/shutdown detection uses weighted ROC + state machine heuristics | Planned |
| REG-13 | High     | `core/regimes.py` | Versioned regime model persistence with validation     | Metadata carries model version; incompatible loads trigger retrain      | Planned |
| REG-14 | High     | `core/regimes.py` | Regime input & config validation                       | Data/config schema issues logged before clustering proceeds             | Planned |
| REG-15 | Medium   | `core/regimes.py` | Sample-based auto-k evaluation                         | Large datasets down-sample for silhouette search without accuracy loss  | Planned |
| REG-16 | Medium   | `core/regimes.py` | Regime reporting enhancements                          | Quality metrics & feature importance exports land in analytics tables   | Planned |

---

## 10 Architecture & Code Quality (FROM STATE MGMT & COMPREHENSIVE AUDITS 2025-11-13)

**Sources:** State Management Audit ACM Main.md, Comprehensive Audit of acm main.md

### 10.1 State Management Improvements

| ID      | Priority | Module                  | Task                                      | Completion Criteria        | Status   |
| ------- | -------- | ----------------------- | ----------------------------------------- | -------------------------- | -------- |
| ARCH-01 | Medium   | `core/acm_main.py`      | Refactor main() into PipelineState class  | Main function < 200 lines  | Deferred |
| ARCH-02 | High     | `core/acm_main.py`      | Implement stage-based pipeline architecture | Stages testable independently | Planned |
| ARCH-03 | High     | `core/acm_main.py`      | Add state validation guards               | State preconditions enforced | Planned |
| ARCH-04 | Medium   | `core/acm_main.py`      | Implement state snapshots for debugging   | Can capture/restore state   | Planned |

### 10.2 Code Quality Fixes

| ID      | Priority | Module             | Task                                               | Completion Criteria                                       | Status  |
| ------- | -------- | ------------------ | -------------------------------------------------- | --------------------------------------------------------- | ------- |
| QUAL-01 | High     | `core/acm_main.py` | Fix O(n²) fallback in episode index mapping        | Always O(n log n) with sorted index requirement           | Planned |
| QUAL-02 | High     | `core/acm_main.py` | Add SQL connection pool limits                     | Max connections = 10 per run                              | Planned |
| QUAL-03 | Medium   | `core/acm_main.py` | Reduce McCabe complexity of main()                 | Complexity < 50                                           | Planned |

---

**END OF TASK BACKLOG - CONSOLIDATED FROM ALL AUDITS (2025-11-13)**
...
| **FCST-06** | Medium | **Vectorized AR(1) Forecast** | `core/forecast.py` | Replace scalar loop with closed-form: `y_{t+h} = � + f^h(y_t - �)` for entire horizon | 2-5x faster for long horizons (>50 steps) | Done | Replaced iterative forecast loop with vectorized closed-form: yhat = � + f^h(y_t - �). Uses np.power for f^h computation. Cleaner code, faster execution. |
| **FCST-07** | Medium | **Forecast Metrics Export** | `core/forecast.py` | Emit `tables/forecast_metrics.csv` with (f, �, s, horizon, series_used, NaN_rate) per run | CSV generated for QA regression testing | Done | Exports comprehensive metrics: AR(1) parameters, horizon, frequency, series selection method, NaN rate, variance. Validated on FD_FAN run. |
