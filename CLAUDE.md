...
27. [Per-Asset Scoring & SIMACM Flow (2026-06-16)](#per-asset-scoring--simacm-flow-2026-06-16)
...
25. *(2026-06-15)* **Operator tab layout overhaul**  "Top Alarm Causes" moved to right column directly below Fleet Health History (causes now in row 2 beside kpis, matrix takes full-width row 3 with `1fr` height). Fleet Health History bars increased 90130px, legend font 911px. Alarm Causes bars 510px, label 1113px bold, count 911px. Grid: `auto auto 1fr` rows with `kpis` spanning rows 1-2 via grid-template-areas.
...
- **v0.2  Simulator Integration** (closed)  SIM tab, SIMACM flow, per-asset scoring
...
- Session 01UuCboiW9MAKb9AKYYoVt1J (2026-06-16): per-asset scoring, SIMACM flow, perf sprints
...
| `table.data` (general) | 1214px | 1618px |
...
**Fixes applied:**
1. **Font sizes:** All mega-matrix elements now 1617px (headers 17px, data rows 17px, episodes 1617px)
2. **Column widths (expanded by 2030px total):**
   - Asset name: 180200px min, 260280px max
   - Status: 90110px
   - Trend: 130150px
   - Fused: 7090px
   - Diagnosis: 220240px
...
3. **Row heights:** Header 3338px, asset rows padding 1012px vertical
4. **Timeline blocks:** 16px20px height for better visibility
5. **Farm group header:** 12px17px font, padding 68px
...
5. O(n2) farm grouping via repeated fleet.filter() calls (5-10ms)
...
- Pre-computed alarmsByAsset map (eliminates O(n2) renderTimeline lookups)
- Pre-parsed dates into dateCache (eliminates 2,400+ Date constructor calls in loops)
- Pre-computed farm groups (eliminates O(n2) filtering)
...
## Per-Asset Scoring & SIMACM Flow (2026-06-16)
> *Added: 2026-06-16*
...
- **Contamination filter**  removing it lowered the self-tuned `alert_z` threshold by ~7% on
  anomaly events (4.2933.977) and ~6% on normal events (3.9643.718), and raised the false-alarm
...
- **OMR**  removing it raised mean fused_max by ~+0.5z on BOTH anomaly (5.5486.070) and normal
  (5.2755.779) events, with the self-tuned alert_z threshold rising in lockstep (so the binary KPI
...
- Runtime was flat across all 5 configs (45.547.2s mean per event)  none of these toggles
...
peak RSS per event. For a full-farm `care_benchmark.py` run, use `--workers 2` (24.5GB  9GB,
...
| precision | 0.857 | 0.923 (false positives 21) |
| F1 | 0.923 | 0.960 |
...
| KPI | FAIL | FAIL (need recall0.80, F10.75) |

False alarms dropped 93 and the GMM-driven false-alarm/broken-baseline pattern that dominated the
...
zero  the 25th75th percentile IQR can collapse to near-zero even though the column has genuine,
...
Dividing by a near-zero IQR amplifies ordinary variation into z-magnitudes of 1e81e12+. Since PCA
...
Farm C's `gmm_z` calibration sanity (`frac3.0` on training data) was ~6x worse than Farm A's even
...
| `gmm_z` train `frac3.0` | (was ~6x worse on Farm C) | now 0.013 vs 0.011  nearly identical |
...
Event 17's score-window `omr_z` (mean 2.93, median 2.30, 43% of rows >3) is substantially elevated
relative to its own training baseline (mean 0.57, median 0.09, 7.8% >3)  a genuine,
...
on several events (e.g. event 35: 7.549.16, event 76: 8.258.91)  OMR now contributes real
...
- Recall jumped +5 events (1621)  OMR firing at all (it structurally could not before the
...
(`/tmp/diag_r2_inspect.py`, not committed) that computed in-sample R2 per engineered feature
...
- **One `omr_z`-driven false alarm did NOT clear**: event 8 (`+heads:omr_z` both before and after,
  `fused_max` literally identical 7.4097.409). This event's elevated `omr_z` is evidently coming
...
- **Two anomaly events flipped from detected  missed**:
  - Event 47: was `+heads:omr_z` (alarm_frac 0.831), now alarm_frac 0.000, empty `rule_fired`.
    `fused_max` barely moved (8.5718.470) but the per-head `omr_z` rule itself no longer clears
  ...
  - Event 90: was `+rate` (not `omr_z` per-head at all), now alarm_frac 0.000. `fused_max` dropped
    9.5748.924 and the self-tuned `alert_z_eff` dropped in lockstep (8.1807.400)  OMR's lower
...
improvement is substantial (115 false alarms, +0.136 precision) and directly validates the
...
in the right direction (0.7120.745) but the farm-wide KPI remains FAIL on both legs. This was
