...
38. *(2026-06-16)* **PowerShell `Start-Process` cannot redirect stdout and stderr to the same file** — `Start-Process` with `-RedirectStandardOutput` and `-RedirectStandardError` pointing to the same path throws `InvalidOperationException: This command cannot be run because "RedirectStandardOutput" and "RedirectStandardError" are same.` Always use **separate** file paths: `$_smokeOut` and `$_smokeErr`, never the same variable for both. This is a hard PS1 constraint with no workaround — always split them.
