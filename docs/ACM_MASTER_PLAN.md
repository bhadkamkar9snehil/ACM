...
- the next coldstart-cleanup slice should now target the remaining non-representation runtime callers that still materially branch on `coldstart_complete`; the baseline-governor surface, persistence path, health stage, and representation path are now cleaned up, so the remaining work is concentrated in `core/smart_coldstart.py` result bundling / compatibility wrappers and any legacy lifecycle/runtime wrappers that still treat the old boolean as operative truth
