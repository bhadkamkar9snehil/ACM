...
- the remaining load-stage coldstart boolean is now gone too: `core/baseline_governor.py::ColdstartLoadDecision` now carries an explicit `use_existing_models` decision plus `runtime_mode_hint`, and `core/smart_coldstart.py` now branches on that explicit load-path contract instead of treating `needs_coldstart` as the operative runtime truth
...
- a later direct governed rerun on that same `WFA_TURBINE_0` window proved the new raw-preview slice is live but not yet universally decisive: ACM now loads `ACM_RegimeState`, runs a read-only raw regime-context preview before full feature preparation, and reuses the seasonality stage instead of rerunning it, but on that window the final decisive blockers still arrived on the later cached-model preview path
...
- the latest parallel governed validation probe on the same three assets, recorded respectively as `RunID=e50175be-a412-4fa3-8882-418a7811ea7a`, `RunID=71835aa3-81ae-4b70-ab72-65dbf8759738`, and `RunID=34622f4e-c334-4575-8bea-898479b2c678`, confirmed that replacing the last load-stage `needs_coldstart` boolean with the explicit `use_existing_models` / `runtime_mode_hint` contract is also behavior-preserving: all three assets still short-circuited before feature/detector/regime work, all three persisted `RuntimeMode=BASELINE_FORMATION`, `BaselineCandidateState=TRUSTED_WINDOW_PENDING`, `ShadowRefreshState=WAITING_FOR_TRUSTED_WINDOW`, `ScoreAllowed=0`, `LearnAllowed=0`, and all three wrote zero score rows
...
- the next runtime slice should therefore target earlier pre-feature / pre-model reduction through extracted owners, not more post-score cleanup and not more basis-churn fixes
- the next baseline-governance slice should keep demoting legacy coldstart authority: replace more of the overloaded use of `coldstart_complete`, keep moving readiness ownership into `core/baseline_governor.py`, and finish deleting stale compatibility references to the removed score-head / score-split fallback path
- the next coldstart-cleanup slice should push those governed readiness hints deeper than persistence and the representation path: any remaining health/adaptation/runtime callers should consume governed readiness fields before falling back to `coldstart_complete`
- the next coldstart-cleanup slice should now target the remaining non-representation runtime callers that still materially branch on `coldstart_complete`; the baseline-governor surface, persistence path, health stage, representation path, public load-stage result, and dead smart-coldstart baseline wrappers are now cleaned up, so the remaining work is concentrated in legacy lifecycle/runtime wrappers and any residual smart-coldstart internals that still encode old coldstart semantics instead of explicit governed readiness
