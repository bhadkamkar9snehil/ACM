...
- the next coldstart-cleanup slice should push those explicit load-stage readiness hints deeper than the representation path itself: persistence, health/adaptation helpers, and any remaining runtime callers should consume governed readiness fields before falling back to `coldstart_complete`
