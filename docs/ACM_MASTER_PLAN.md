...
- Slice 11 completed: authoritative representation gating now exists in validation mode only, activation is explicit and replay-safe, learning-side effects are blocked when `learn_allowed = false`, and downstream scoring outputs are suppressed when `score_allowed = false` without changing default production authority.
