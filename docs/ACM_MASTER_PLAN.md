...
- the next coldstart-cleanup slice should now target the remaining non-representation runtime callers that still materially branch on `coldstart_complete`; the persistence path, health stage, and representation path are now cleaned up, so the remaining work is concentrated in legacy lifecycle/runtime wrappers and any leftover smart-coldstart result consumers that still treat the old boolean as operative truth
