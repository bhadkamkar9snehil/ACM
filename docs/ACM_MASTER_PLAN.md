...
- the next coldstart-cleanup slice should now target the remaining non-representation runtime callers that still materially branch on `coldstart_complete`, especially baseline-buffer refresh fallback sites and any legacy lifecycle/runtime wrappers that have not yet been replaced by explicit governed readiness fields
