...
- Live in repo:
  ...
  - coldstart replay now prefers governed `ACM_BaselineGovernance.RuntimeMode` when deciding whether coldstart is complete; legacy `ACM_ActiveModels.RegimeMaturityState` is fallback only
...
- the next baseline-governance slice should keep demoting the last legacy lifecycle wrappers: `scripts/sql_batch_runner.py` and `core/smart_coldstart.py` now both prefer governed runtime mode, so the remaining coldstart cleanup is concentrated in retiring fallback-only lifecycle semantics and then pushing more no-score resolution earlier than feature prep
- the next coldstart-cleanup slice should now target remaining prep-cost reduction, not more public boolean cleanup: the baseline-governor surface, persistence path, health stage, representation path, public load-stage result, smart-coldstart helper state, and runner coldstart authority are now cleaned up enough that the main remaining coldstart/baseline work is finishing lifecycle fallback retirement and trusted-window package evolution
