...
sets. We evaluate on the public CARE-to-Compare wind-turbine SCADA benchmark (Farm A, 22
labelled events, 86 sensors; Farm C, 58 labelled events, 957 sensors) with zero per-farm
tuning, and report ablations isolating each design choice's contribution.
...
1. **Contamination-aware calibration** (§3.5) — an iterative MAD-based filter removes
...
2. **A self-distrust gate that discriminates by magnitude, not just timing** (§3.7) — a
...
3. **Cadence-inferred alarm-rule horizons** (§3.7) — every rule horizon is defined in
   physical time and converted to sample counts using the cadence measured directly
   from the scored data's own timestamps.
4. **An interleaved calibration split** (§3.3) — held-out calibration blocks are
...
5. **OMR's top-3 multivariate residual aggregation** (§3.4.6) — a model-agnostic
...
We evaluate ACM end-to-end against the CARE-to-Compare wind-farm benchmark, with the
*same* hyperparameters used across farms of very different scale (Farm A: 86 sensors;
Farm C: 957 sensors) — i.e., zero per-asset, per-farm hand-tuning — and report ablations
that isolate each design choice's measured contribution.
...
trade-off for classifier ensembles; ACM's fusion stage's correlation discount (§3.6) and
...
calibration (§3.5) makes the contamination assumption explicit and addresses it with an
iterative robust filter rather than assuming it away.

**Industrial benchmarks.** CARE-to-Compare is a public, multi-farm wind-turbine SCADA
benchmark with labelled fault events and an explicit train/test split column per asset.
...
benchmark harness used for evaluation in §4–5 — there is no separate "research" code
path. The pipeline has seven stages, described in this section in execution order.

### 3.1 Channel Role Detection
...
feature count (notably GMM; see §3.4.5).
...
Derived channels (§3.1) join the feature matrix unchanged, with no rolling statistics
...
### 3.3 Interleaved Calibration Split

A subsequent calibration stage (§3.5) needs an out-of-sample reference: detector fit
...
the whole timeline rather than concentrated at either end. Detectors (§3.4) are fit on
the remaining `~80%`; calibration (§3.5) and OMR's residual-scale recalibration
(§3.4.6) both use the held-out blocks.

### 3.4 Detector Ensemble
...
calibrated independently into a z-score by the shared calibration stage (§3.5); no
...
| `omr_z` | Multivariate reconstruction residual (top-3 aggregation; §3.4.6) | Captures correlated multi-sensor relationship breakdown and provides per-channel fault attribution |
...
**3.4.1 AR1.** A per-channel order-1 autoregression with smoothing parameter
...
**3.4.2/3.4.3 PCA-SPE / PCA-T².** A single PCA fit (randomized SVD,
...
**3.4.4 Isolation Forest.** `n_estimators = 100`, `max_samples = 2048`,
...
**3.4.5 Gaussian Mixture Model.** Components are selected by BIC search in the range
...
pipeline (AR1's `z_cap`, the shared calibrator's hard z-clip; §3.5).

**3.4.6 OMR (Overall Model Residual).** OMR fits a single multivariate model — not one
...
holdout (§3.3) rather than from the rows the model was fit on: a model is optimized to
...
OMR's raw aggregate score reaches the shared calibration stage (§3.5) unclipped, on the
same terms as every other head — calibration and the universal hard ceiling described in
§3.5 are the only place any head's score is bounded.
...
### 3.5 Contamination-Aware Calibration

Each head's raw score is converted to a calibrated z-score by a shared calibrator fit on
the out-of-sample calibration holdout (§3.3): `z = (x - median) / (MAD × 1.4826)`, with a
...
magnitude criterion (§3.7) is calibrated against directly.

### 3.6 Correlation-Discounted Weighted Fusion
...
### 3.7 Alarm Rules and the Self-Distrust Gate
...
calibrator's universal hard clip at `z = ±10.0` (§3.5) is, in practice, always the
...
**Dataset.** CARE-to-Compare, a public wind-turbine SCADA benchmark (Zenodo record
15846963) with per-event labelled fault windows and an explicit `train_test` split
column per timestamp. We evaluate on Farm A (22 labelled events, 86 sensor channels) and
Farm C (58 labelled events: 31 normal, 27 anomaly; 957 sensor channels) — chosen to
stress-test the same fixed configuration across an order-of-magnitude difference in
sensor count. Farm B is present in the dataset but not evaluated in this work.

**Protocol.** `score_asset()` (§3) is called once per labelled event with that event's
...
### 5.1 Main result — Farm A

[RESULTS PENDING]

### 5.2 Farm C — full-farm KPI

[RESULTS PENDING]

### 5.3 Ablations — Farm A
...
- Farm B is unevaluated. Generality claims rest on two farms differing by sensor count,
  not a third independent farm.
- OMR's top-3 choice (§3.4.6) is justified by failure-mode reasoning (L2 dilution, max
  tail growth).
